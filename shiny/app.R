  ...
  ui <- fluidPage(
    navbarPage("TV Show Ratings",collapsible = T,
               theme = shinytheme("lumen"), id = "navbarID",
               ...
               tabPanel("Shows", fluid = TRUE,value = "page2",
                        fluidRow(
                          ...
                          column(5,sliderInput("years_start",
                                               "Year came out:",
                                               min = 1930, max = 2025, value = c(1990,2023),sep = "",step = 1,ticks = F)),
    ...
    output$p2 <- renderPlotly({
      ...
      p2 <- ggplotly(height = 500, source = "shows_scatterplot",
                     ggplot(data=shows_slopes,
                      ...
                      geom_jitter(pch=20, alpha=0.1,width = 0.05,height = .05)+
                      theme_minimal() +
                      labs(y=" ← Gets worse       Gets better → ", x = "Rating better →")+
                    #scale_x_log10(),
                    scale_y_continuous(limits = c(graph_y_lower,0.95)),
                    tooltip = "text") %>% 
